...
export async function submitAdoptionApplication(
  ...
  try {
    await sendApplicationEmail(
      fields,
      `New Adoption Application — ${name}`,
      "Adoption Application",
    );
  } catch (err) {
    console.error("Adoption application email failed:", err);
    return {
      status: "error",
      message:
  ...
  return {
    status: "success",
    message:
      "Your Adoption Application has been received! We will review it and be in touch soon.",
  };
}
