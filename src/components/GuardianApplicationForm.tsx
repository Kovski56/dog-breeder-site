...
} from "@/app/guardian-program/application/actions";
import { ApplicationFields } from "./ApplicationFormFields";

const initial: ApplicationState = { status: "idle", message: "" };

export default function GuardianApplicationForm() {
  const [state, formAction, pending] = useActionState(submitGuardianApplication, initial);
  ...
  return (
    <form action={formAction} className="space-y-7">
      ...
      >
        {pending ? "Submitting…" : "Submit Guardian Application"}
      </button>
    </form>
  );
}
